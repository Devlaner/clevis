...
Faked GitHub via `patch(".GitHubClient")`.
"""
...
def test_comment_mode_nudges_only_stale_non_draft_prs_and_audits(client, db, user):
    ...
    with patch("src.routers.pr_nudges.GitHubClient") as mock:
        ...
        inst.request.side_effect = lambda method, path, **kw: (
            prs if path.endswith("/pulls")
            else [] if path.endswith("/comments") and method == "GET"
            else {"id": 1}
        )
...
def test_comment_mode_skips_a_pr_already_nudged(client, db, user):
    ...
    with patch("src.routers.pr_nudges.GitHubClient") as mock:
        ...
        inst.request.side_effect = lambda method, path, **kw: (
            prs if path.endswith("/pulls")
            else [{"body": "ping <!-- clevis:pr-nudge -->"}] if path.endswith("/comments")
            else {"id": 1}
        )
        resp = client.post("/me/repos/acme/api/pr-nudges", json={"token": "ghp_admin"})
...
def test_off_mode_makes_no_write_calls(client, db, user):
    _admin_org(db, user)
    set_config("pr_nudge_mode", "off")
    with patch("src.routers.pr_nudges.GitHubClient") as mock:
        mock.return_value.request.return_value = [_pr(1, 30)]
        resp = client.post("/me/repos/acme/api/pr-nudges", json={"token": "ghp_admin"})
    assert resp.status_code == 200
    assert resp.json()["results"] == []
    mock.return_value.request.assert_not_called()
...
def test_label_mode_adds_the_needs_review_label(client, db, user):
    ...
    with patch("src.routers.pr_nudges.GitHubClient") as mock:
        inst = mock.return_value
        inst.request.side_effect = lambda method, path, **kw: [_pr(1, 10)] if path.endswith("/pulls") else {}
        resp = client.post("/me/repos/acme/api/pr-nudges", json={"token": "ghp_admin"})
